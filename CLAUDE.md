...
- **ReportsAPI** — `GET /api/storage/reports/v1/export-excel` (ClosedXML) e `export-pdf` (QuestPDF, licença Community declarada no `Program.cs`), com gráficos do ScottPlot renderizados em PNG. Valor de cada venda é `SalPrice × SalQntd`. Limitações do código atual, cada uma com issue aberta: o filtro do mês corrente compara só `SalDateSale.Month`, **sem o ano**, então junta o mesmo mês de anos anteriores (#15); a quantidade é a bruta, **sem descontar devoluções**, e vendas totalmente devolvidas aparecem (#16); e a API devolve o arquivo sem extensão — pela tela não se nota, porque o Insights baixa com o próprio nome e extensão, mas quem chama o endpoint direto recebe o arquivo sem ela (#17). Na tela, erro de relatório não chega ao usuário e erro da IA aparece como resposta no chat (#18). O resumo por IA dentro do PDF está comentado — é a issue #5.
