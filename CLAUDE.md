...
- **ReportsAPI** — `GET /api/storage/reports/v1/export-excel` (ClosedXML) e `export-pdf` (QuestPDF, licença Community declarada no `Program.cs`), com gráficos do ScottPlot renderizados em PNG. Valor de cada venda é `SalPrice × SalQntd`, e o recorte é o mês corrente do ano corrente, pelo `SalDateSale`. Limitação do código atual, com issue aberta: a quantidade é a bruta, **sem descontar devoluções**, e vendas totalmente devolvidas aparecem (#16). Na tela, erro de relatório não chega ao usuário e erro da IA aparece como resposta no chat (#18). O resumo por IA dentro do PDF está comentado — é a issue #5.
